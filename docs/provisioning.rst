...
Rapidsms_Pycon is deployed on the following stack.
...
As documented you should create/import your Celery app in ``rapidsms_pycon/__init__.py`` so that you
can run the worker via::

    celery -A rapidsms_pycon worker

Additionally you will need to configure the project settings for Celery::

    # rapidsms_pycon.settings.staging.py
    import os
    from rapidsms_pycon.settings.base import *

    # Other settings would be here
    BROKER_URL = 'amqp://rapidsms_pycon_staging:%(BROKER_PASSWORD)s@%(BROKER_HOST)s/rapidsms_pycon_staging' % os.environ

You will also need to add the ``BROKER_URL`` to the ``rapidsms_pycon.settings.production`` so
...
RabbitMQ user will be named rapidsms_pycon_<environment> and the vhost will be named rapidsms_pycon_<environment>
...
You can use the below OpenSSL commands to generate the key and signing request::
  ...
  openssl genrsa -out rapidsms_pycon.key 2048
  # Make copy of the key with the passphrase
  cp rapidsms_pycon.key rapidsms_pycon.key.secure
  # Remove any passphrase
  openssl rsa -in rapidsms_pycon.secure -out rapidsms_pycon.key
  # Generate signing request
  openssl req -new -key rapidsms_pycon.key -out rapidsms_pycon.csr
